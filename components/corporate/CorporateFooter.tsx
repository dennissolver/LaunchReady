        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span>{productName ? (<>{productName} — Built by{' '}<span className={isDark ? 'text-white font-medium' : 'text-slate-900 font-medium'}>Corporate AI Solutions</span></>) : (<>Built by{' '}<span className={isDark ? 'text-white font-medium' : 'text-slate-900 font-medium'}>Corporate AI Solutions</span></>)}</span>
            </div>
            <div className="flex flex-wrap items-center justify-center gap-4 text-sm">
              ...
              <a href="https://www.calendly.com/mcmdennis" target="_blank" rel="noopener noreferrer" className="hover:text-slate-900 transition-colors">Book a Call</a>
              <span className="hidden sm:inline opacity-30">|</span>
              <a href="tel:+61402612471" className="hover:text-slate-900 transition-colors">+61 402 612 471</a>
              <span className="hidden sm:inline opacity-30">|</span>
              <a href="mailto:dennis@corporateaisolutions.com" className="hover:text-slate-900 transition-colors">dennis@corporateaisolutions.com</a>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs opacity-70">
            ...
            {extraLinks.length > 0 && (
              <div className="flex gap-4">
                {extraLinks.map((link) => link.external ? (<a key={link.href} href={link.href} target="_blank" rel="noopener noreferrer" className="hover:text-slate-900 transition-colors">{link.label}</a>) : (<a key={link.href} href={link.href} className="hover:text-slate-900 transition-colors">{link.label}</a>))}
              </div>
            )}
          </div>
        </div>
