...
// DATAWIZZ - Smart LLM Router
const openai = new OpenAI({
  apiKey: process.env.DATAWIZZ_API_KEY!,
  baseURL: process.env.DATAWIZZ_BASE_URL || 'https://gw.datawizz.app/b413b55882a3eb6b/openai/v1',
})

const DATAWIZZ_ENDPOINT = process.env.DATAWIZZ_ENDPOINT || 'laucnhready'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!process.env.DATAWIZZ_API_KEY) {
      return NextResponse.json({ error: 'DATAWIZZ not configured' }, { status: 500 })
    }
    ...
    // Add context data (current form values, project data, etc.)
    if (contextData) {
      messages.push({
        role: 'system',
        content: `CURRENT PROJECT DATA:\n${JSON.stringify(contextData, null, 2)}\n\nUse this to avoid asking for information already provided and to give accurate advice.`,
      })
    }
    ...
    // Call DATAWIZZ (routes to optimal LLM)
    const completion = await openai.chat.completions.create({
      model: DATAWIZZ_ENDPOINT,
      messages,
      max_tokens: 500,
      temperature: 0.4,
      // @ts-ignore - DATAWIZZ supports metadata for logging/routing
      metadata: {
        task: 'voice-extraction',
        context,
        project: 'launchready',
      },
    })
